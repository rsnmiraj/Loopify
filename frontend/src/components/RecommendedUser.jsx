	...
	const renderButton = () => {
		...
		switch (connectionStatus?.data?.status) {
			...
			default:
				return (
					<button
					className="px-3 py-1 rounded-full text-sm border border-primary text-white hover:bg-black hover:text-white transition-colors duration-200 flex items-center"
	...
	return (
		<div className='flex items-center justify-between mb-4'>
			<Link to={`/profile/${user.username}`} className='flex items-center flex-grow'>
				...
				/>
				<div>
					<h3 className='font-bold text-sm text-teal-300'>{user.name}</h3>
					<p className='text-xs text-amber-400 '>{user.headline}</p>
				</div>
			</Link>
			{renderButton()}
		</div>
